...
4B03 的应用工厂会根据该变量集中选择适配器：

- `true`：创建 Mock 适配器，不创建 HTTP 客户端，不产生任何 MaxKB 网络请求；
- `false`：创建持有私有 HTTPX 客户端的 HTTP 适配器，但应用启动和健康检查仍不会请求 MaxKB；
- 两种模式都由 FastAPI 应用生命周期统一关闭，不允许路由自行创建 HTTPX 客户端；
- 健康检查只反映自研后端是否可以响应，不探测 MaxKB 可用性。

当前 HTTP 实现只是连接资源和切换位置的骨架，尚未定义认证头、接口路径及问答、生成、评价的请求响应契约，因此不能用于真实业务调用。后续完成已发布应用的 API 契约、适配器方法和测试后，再确保地址、API Key 与应用标识均已配置，并将其改为：
